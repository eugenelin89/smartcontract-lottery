    ...
    : describe("Raffle Unit Tests", function () {
          ...
          describe("enterRaffle", function () {
              ...
              it("doesn't allow entrance when raffle is calculating", async function () {
                  ...
                  await network.provider.send("evm_mine", []) // same thing: await network.provider.request({method:"evm_mine", params:[]})
          ...
          describe("checkUpkeep", function () {
              it("returns false if people haven't sent any ETH", async function () {
                  ...
                  // calling await raffle.checkUpKeep([]) will kick off a transaction
              ...
              it("return false if enough time hasn't past", async function () {
                  await raffle.enterRaffle({ value: raffleEntranceFee })
                  await network.provider.send("evm_increaseTime", [interval.toNumber() - 1])
                  await network.provider.send("evm_mine", [])
                  const { upkeepNeeded } = await raffle.callStatic.checkUpkeep([])
